# Cantidad Clientes Al Corte De Un Mes
...
```sql
---CREATE OR REPLACE TABLE sb-sandbox-usuarios.sandbox_cumplimiento.cifras_conocimiento_cliente AS

INSERT INTO sb-sandbox-usuarios.sandbox_cumplimiento.cifras_conocimiento_cliente 
-- ✅ Parámetro de corte editable
WITH parametros AS (
    SELECT DATE('2025-01-31') AS FECHA_CORTE_CALCULO  -- <<< CAMBIA AQUÍ LA FECHA DE CORTE CUANDO LO NECESITES  
    #SELECT DATE('2025-02-28') AS FECHA_CORTE_CALCULO  -- <<< CAMBIA AQUÍ LA FECHA DE CORTE CUANDO LO NECESITES  
    #SELECT DATE('2025-03-31') AS FECHA_CORTE_CALCULO  -- <<< CAMBIA AQUÍ LA FECHA DE CORTE CUANDO LO NECESITES
    #SELECT DATE('2025-04-30') AS FECHA_CORTE_CALCULO  -- <<< CAMBIA AQUÍ LA FECHA DE CORTE CUANDO LO NECESITES
    #SELECT DATE('2025-05-31') AS FECHA_CORTE_CALCULO  -- <<< CAMBIA AQUÍ LA FECHA DE CORTE CUANDO LO NECESITES
    #SELECT DATE('2025-06-30') AS FECHA_CORTE_CALCULO  -- <<< CAMBIA AQUÍ LA FECHA DE CORTE CUANDO LO NECESITES
...
clientes_unificados AS (
    SELECT DISTINCT 
        ...
        IFNULL(PUBLICAMENTE_EXPUESTA, 'N') AS PUBLICAMENTE_EXPUESTA
...
polizas_vigentes AS (
    ...
    FROM (
        ...
        JOIN parametros prm ON DATE(c.fecha_corte) = prm.FECHA_CORTE_CALCULO
    )
...
GROUP BY 
    ...
    estado_actualizacion
ORDER BY 
    CODIGO_COMPANIA, tipo_persona, Nivel_de_Riesgo,PUBLICAMENTE_EXPUESTA, estado_actualizacion;
