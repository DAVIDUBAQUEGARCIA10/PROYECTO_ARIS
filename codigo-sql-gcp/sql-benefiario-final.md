...
------------------------------------
Cuantificacion de poblamiento en core de beneficiario final - y con usuario  
...
/* ================================================================
   RESUMEN: ¿Cuántos NIT únicos caen en cada CLASIFICACION_PARTICIPACION?
   - Se limita al universo filtrado en MAIN (respeta :P_NIT si lo envías).
   - Incluye % sobre el total.
   ================================================================ */

WITH
/* Reusa EXACTAMENTE tus CTE previas... */
MAIN AS (
  SELECT DISTINCT exrf_num_id_cliente AS NIT
  FROM dc_ext_revisoria_fiscal
  WHERE exrf_cod_tipo_persona = 'J'
...
/* ---- Conteo de NIT únicos por categoría (acotado a MAIN) ---- */
CLASIF_NIT AS (
  SELECT
    s.CLASIFICACION_PARTICIPACION,
    COUNT(DISTINCT s.NUMERO_DOCUMENTO) AS NITS_UNICOS
  FROM SUM_PARTICIPACION s
  JOIN MAIN m
    ON m.NIT = s.NUMERO_DOCUMENTO
  GROUP BY s.CLASIFICACION_PARTICIPACION
),
TOTAL AS (
  SELECT SUM(NITS_UNICOS) AS TOTAL_NITS FROM CLASIF_NIT
)
SELECT
  c.CLASIFICACION_PARTICIPACION,
  c.NITS_UNICOS,
  ROUND(100 * c.NITS_UNICOS / NULLIF(t.TOTAL_NITS, 0), 2) AS PORCENTAJE_SOBRE_TOTAL
FROM CLASIF_NIT c
CROSS JOIN TOTAL t
ORDER BY
  CASE c.CLASIFICACION_PARTICIPACION
    WHEN 'SIN ACCIONISTAS'      THEN 1
    WHEN 'Menor a 80%'          THEN 2
    WHEN 'Entre 80% y 100%'     THEN 3
    WHEN 'Mayor a 100%'         THEN 4
