# Revisión De Recaudos Para Plantilla De Efectivo
...
-- Filtro por fechas de asiento contable en junio de 2025
WHERE 
    m1.FEC_ASIENTO BETWEEN '01/06/25' AND '30/06/25'

-- Tipo de actuación CB (Compensación bancaria, posiblemente)
    AND m1.TIPO_ACTU = 'CB'

-- Valor igual o superior a 10 millones
    AND m1.IMP_MON_PAIS >= 10000000

-- Solo registros en el debe
    AND m1.MCA_DEBE_HABER = 'D'

-- Que tenga sección definida
    AND m1.COD_RAMO IS NOT NULL

-- Excluye un recibo específico
  AND m1.RECIBO != 9999095 -- Esto es ingresos barras 4000000207411440
   AND m1.RECIBO != 9999996 -- Esto es finanaseguros
-- Solo para estas cuentas específicas
...
-- Excluye un beneficiario específico
    AND m1.COD_BENEF != 860034313
    and COD_PAGO = 2
   and COD_CIA = 2
-- Solo talonarios con longitud entre 2 y 4 caracteres (inclusive)
   AND m1.NUMERO_TALON >= 100 AND m1.NUMERO_TALON < 10000
   and m1.nombre_entidad_talon = 'BANCA DAVIVIENDA'
