# Transaccionalidad Polizas Solo Terceros Restringidos
...
-- Paso 1: Pre-filtrar restringidos activos UNA sola vez
RESTRINGIDOS AS (
    SELECT 
        cr.NUMERO_DOCUMENTO,
        cr.TIPDOC_CODIGO,
        cr.NOMBRES          AS NOMBRE_RESTRINGIDO,
        fr.CODIGO           AS CODIGO_RESTRICCION,
...
-- Paso 2: Consolidar movimientos solo de clientes restringidos
CONS AS (
    SELECT 
        TDOC_TERCERO,
        COD_BENEF,
    ...
    FROM A5021604
    WHERE FEC_ASIENTO >= TO_DATE('01/01/2026', 'DD/MM/YYYY')
      AND IMP_MON_PAIS IS NOT NULL
      -- Pre-filtro temprano: solo restringidos
      AND EXISTS (
            SELECT 1 FROM RESTRINGIDOS r
            WHERE r.NUMERO_DOCUMENTO = COD_BENEF
              AND r.TIPDOC_CODIGO    = TDOC_TERCERO
      )
    ...
    FROM A5021900
    WHERE FEC_ASIENTO >= TO_DATE('01/01/2026', 'DD/MM/YYYY')  -- ⚠️ estaba en 2020, ajusta si es intencional
...
-- Paso 3: Enriquecer y deduplicar
CTE AS (
    SELECT
        CONS.TDOC_TERCERO,
        CONS.COD_BENEF,
        ...
        R.NOMBRE_RESTRINGIDO,
        R.CODIGO_RESTRICCION,
        R.DESCRIPCION_RESTRICCION,
        ROW_NUMBER() OVER (
            PARTITION BY CONS.VALOR_MOVIMIENTO, CONS.COD_BENEF, CONS.NUM_POL1,
                         CONS.COD_OFIC_CONTAB,  CONS.FEC_ASIENTO
            ORDER BY A200.FECHA_EMI DESC
        ) AS rn
    FROM CONS
    -- Traer datos de restricción ya calculados (sin re-leer las tablas)
    JOIN RESTRINGIDOS R
        ON  CONS.COD_BENEF    = R.NUMERO_DOCUMENTO
        AND CONS.TDOC_TERCERO = R.TIPDOC_CODIGO
    LEFT JOIN A2000030 A200
        ON CONS.NUM_POL1 = A200.NUM_POL1
    LEFT JOIN A1001800 b
        ON A200.cod_end = b.cod_texto AND A200.sub_cod_end = b.sub_cod_texto
    LEFT JOIN NATURALES NAT
        ON CONS.COD_BENEF = NAT.NUMERO_DOCUMENTO AND CONS.TDOC_TERCERO = NAT.TIPDOC_CODIGO
    LEFT JOIN JURIDICOS JUR
        ON CONS.COD_BENEF = JUR.NUMERO_DOCUMENTO AND CONS.TDOC_TERCERO = JUR.TIPDOC_CODIGO
    WHERE CONS.TDOC_TERCERO    IS NOT NULL
      AND CONS.VALOR_MOVIMIENTO > 0
)
