# Cliente Juridico - Seguros Bolivar
...
```
-- =====================================================================
-- Tabla final: segmentacion_seguros_bolivar_pj_cliente
-- Alcance:
--   - Integra variables de cliente jurídico, score, siniestros y recaudos (12M)
...
CREATE OR REPLACE TABLE
  `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.segmentacion_seguros_bolivar_pj_cliente` AS
...
-- ---------------------------------------
-- 1) Pólizas base (PJ activas y no anuladas)
-- ---------------------------------------
polizas AS (
  SELECT DISTINCT
  ...
  WHERE DATE(p.FECHA_FIN_POLIZA) >= CURRENT_DATE()
    ...
    AND p.CODIGO_RAMO_EMISION != 310
    AND UPPER(p.TIPO_DOCUMENTO_TOMADOR) IN ("NE","NT") -- Jurídicos
),

-- ----------------------------
-- 2) Clientes jurídicos
-- ----------------------------
clientes AS (
  SELECT
...
WHERE VALOR_RECAUDO_ULT_12_MESES   >= 0
  AND VALOR_SINIESTROS_ULT_12_MESES >= 0
-- Opcional: activar filtros de percentiles
...
Archivo Original: Cliente Juridico - Seguros Bolivar
