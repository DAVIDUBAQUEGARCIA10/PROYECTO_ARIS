# Cliente Natural - Seguros Bolivar
...
-- =====================================================================
-- Tabla final: segmentacion_seguros_bolivar_pn_cliente
-- Alcance:
--   - Integra variables de cliente, profesión/ocupación, score, siniestros y recaudo (12M)
--   - Normaliza llaves (compañía, tipo doc, key_id) para evitar no-matches
--   - Expone métricas agregadas y percentiles (con opción de apagar filtro)
-- =====================================================================

CREATE OR REPLACE TABLE
  `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.segmentacion_seguros_bolivar_pn_cliente` AS
...
WITH limites AS (
  SELECT
    -- Últimos 12 meses completos (mes calendario), excluyendo el mes en curso
...
-- ---------------------------------------
-- 1) Pólizas base (PN activas y no anuladas)
-- ---------------------------------------
polizas AS (
  SELECT distinct
  ...
  WHERE DATE(p.FECHA_FIN_POLIZA) >= DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH) -- vigentes desde hace 12 meses atras sin contar el mes actual
    AND SAFE_CAST(p.CODIGO_COMPANIA AS INT64) = 2 
    and CODIGO_RAMO_EMISION != 310               -- compañía 2
    AND UPPER(p.TIPO_DOCUMENTO_TOMADOR) NOT IN ('NE','NT')       -- PN (excluye NIT/extranjeros empresa)
),

-- ----------------------------
-- 2) Clientes naturales (enriq)
-- ----------------------------
clientes AS (
  SELECT
...
-- -----------------------------------------
-- 4) Catálogos: ocupación y profesión (riesgo)
-- -----------------------------------------
ocupacion AS (
  SELECT
    SAFE_CAST(o.ID AS INT64) AS COD_OCUPACION,
    o.OCUPACION
  FROM `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.t_ocupacion_nivel_riesgo_real` o
),
profesion AS (
  SELECT
    SAFE_CAST(pr.ID AS INT64) AS COD_PROFESION,
    pr.PROFESION,
    pr.NIVEL_DE_RIESGO AS NIVEL_RIESGO_PROFESION
  FROM `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.t_profesion` pr
),

-- -----------------------------------------------------
-- 5) Siniestros últimos 12 meses (llaves + fecha robusta)
--     OJO: Si FECHA_ORDEN_PAGO está poco poblada, ampliamos COALESCE
-- -----------------------------------------------------
siniestros_12m AS (
    SELECT distinct 
    ...
    WHERE
      ...
      DATE(COALESCE(t.FECHA_ORDEN_PAGO, t.FECHA_MOVIMIENTO)) BETWEEN
        DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH), MONTH)
        AND DATE_SUB(DATE_TRUNC(CURRENT_DATE(), MONTH), INTERVAL 1 DAY)
      AND SAFE_CAST(t.CODIGO_COMPANIA AS INT64) = 2
      AND t.CODIGO_RAMO_EMISION != 310
      AND IFNULL(t.LIQUIDADO_BOLIVAR,0) > 0
      -- normaliza la llave IGUAL que en el SELECT
    GROUP BY 1,2,3
),

-- ----------------------------------------------------
-- 6) Recaudos últimos 12 meses (llaves + fecha explícita)
-- ----------------------------------------------------
recaudos_12m AS (
  SELECT distinct 
  ...
  WHERE DATE(r.FECHA_RECAUDO) BETWEEN l.f_ini_12m AND l.f_fin_12m
    ...
    AND SAFE_CAST(r.CODIGO_COMPANIA AS INT64) = 2
    AND IFNULL(R.VALOR_RECAUDO_MES,0) > 0
  GROUP BY 1,2,3
),

-- ---------------------------------------------------------
-- 7) Integración (LEFT JOINs sobre llaves normalizadas)
-- ---------------------------------------------------------
datos_agregados AS (
  SELECT DISTINCT
    -- Llaves
    ...
    -- Profesión / ocupación
    pr.PROFESION,
    pr.NIVEL_RIESGO_PROFESION,

    -- Score
    IFNULL(s.NIVEL_DE_RIESGO, 'Sin Nivel Riesgo') AS NIVEL_DE_RIESGO,

    -- Siniestros (protegidos con IFNULL)
    ...
    -- Normalizaciones varias (tipo doc, ocupación, profesión) – conservadas de tu lógica
    CASE p.TIPO_DOCUMENTO_TOMADOR
      WHEN 'CC' THEN 1 WHEN 'NT' THEN 2 WHEN 'CE' THEN 3 WHEN 'NE' THEN 4
      WHEN 'TI' THEN 5 WHEN 'CD' THEN 6 WHEN 'PE' THEN 7 ELSE 0 END AS tipo_doc_normalizado,

    CASE c.COD_OCUPACION
      ...
      WHEN 11 THEN 11 WHEN 0 THEN 0 ELSE 12 END AS COD_OCUPACION_NUM,

    CASE c.COD_PROFESION
      WHEN 0 THEN 1 WHEN 28 THEN 2 WHEN 56 THEN 3 WHEN 86 THEN 4 WHEN 94 THEN 5
      WHEN 218 THEN 6 WHEN 130 THEN 7 WHEN 108 THEN 8 WHEN 210 THEN 9 ELSE 10 END AS COD_PROFESION_NUM

    -- (El CASE masivo de CIIU y mapeos numéricos se aplican abajo para claridad)
  FROM polizas p
  LEFT JOIN clientes  c  USING (TIPO_DOCUMENTO_TOMADOR, KEY_ID_TOMADOR)
  LEFT JOIN ocupacion o  ON c.COD_OCUPACION = o.COD_OCUPACION
  LEFT JOIN profesion pr  ON c.COD_PROFESION = pr.COD_PROFESION
...
-- ----------------------------------------------------------------------
-- 8) Enriquecimiento: CIIU_NUM, riesgos numéricos y percentiles (opcional)
-- ----------------------------------------------------------------------
enriquecido AS (
  SELECT
    d.*,

    -- ====== TU MEGAMAPEADOR CIIU A NÚMERO ======
    /* Conservado íntegro tal cual (abreviado aquí para foco) */
    CASE CODIGO_CIIU
      WHEN 7110 THEN 1
      WHEN 10 THEN 2
      WHEN 1313 THEN 3
    ...
    -- Percentiles para winsorizar outliers (puedes apagar el filtro abajo)
    NTILE(100) OVER (ORDER BY d.SUMA_RECAUDO_ULT_12_MESES)  AS percentil_rank_r,
    NTILE(100) OVER (ORDER BY d.SUMA_LIQUIDADO_ULT_12_MESES) AS percentil_rank_s

  FROM datos_agregados d
)

-- -------------------------------------------------
-- 9) SELECT final (con opción de filtro percentiles)
-- -------------------------------------------------
SELECT
  ...
  IFNULL(SAFE_DIVIDE(VALOR_ACTIVO,   NULLIF(VALOR_PASIVO, 0)),   0) AS ACTIVO_PASIVO,
  IFNULL(SAFE_DIVIDE(VALOR_INGRESOS, NULLIF(VALOR_EGRESOS, 0)),  0) AS INGRESO_EGRESO,

  CODIGO_CIIU_NUM,
  NIVEL_RIESGO_PROFESION_NUM,
  NIVEL_DE_RIESGO_NUM,

  -- Siniestros (expuestos como pediste)
  CANTIDAD_SINIESTROS_ULT_12_MESES AS CANTIDAD_SINIESTRO_12,
  SUMA_LIQUIDADO_ULT_12_MESES      AS SUMA_SINIESTRO_12,

  -- Recaudos
  SUMA_RECAUDO_ULT_12_MESES        AS SUMA_RECAUDO_12,
  CANTIDAD_RECAUDOS_ULT_12_MESES   AS CANTIDAD_RECAUDO_12

FROM enriquecido
-- >>> Si tu muestra queda vacía, PRIMERO desactiva temporalmente los filtros siguientes <<<
WHERE 1=1
...
Archivo Original: Cliente Natural - Seguros Bolivar
