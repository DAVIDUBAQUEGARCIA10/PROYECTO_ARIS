...
```
CREATE OR REPLACE TABLE sb-sandbox-usuarios.sandbox_cumplimiento.cambio_segmento_pj AS
...
WITH historico_ordenado AS (
  SELECT 
    ...
    -- Segmento anterior
    LAG(SEGMENTO) OVER (
      PARTITION BY COMPANIA, TIPO_DOCUMENTO_TOMADOR, KEY_ID_TOMADOR
  ...
  FROM `sb-sandbox-usuarios.sandbox_cumplimiento.union_pj_modelo_segmentacion`
),
...
WHERE (
  (SEGMENTO = "cluster_3" AND COMPANIA = 2) OR
  (SEGMENTO = "cluster_9" AND COMPANIA = 2) OR
  (SEGMENTO = "cluster_15" AND COMPANIA = 2) OR
  (SEGMENTO = "cluster_18" AND COMPANIA = 2) OR
  (SEGMENTO = "cluster_21" AND COMPANIA = 2) OR      
  (SEGMENTO = "cluster_23" AND COMPANIA = 3) OR
  (SEGMENTO = "cluster_24" AND COMPANIA = 3) OR
  (SEGMENTO = "cluster_2" AND COMPANIA = 1) OR
  (SEGMENTO = "cluster_8" AND COMPANIA = 1) 
)
...
ORDER BY COMPANIA, TIPO_DOCUMENTO_TOMADOR, KEY_ID_TOMADOR, FECHA;
