...
-- 3) Query principal: PRIMAS + SINIESTROS
SELECT
  -- Llave de agregación (PRIMAS)
  ...
  -- Métricas de SINIESTROS (pueden ser NULL si no hay match)
  IFNULL(s.SINIESTRO_LIQUIDADO_TOTAL, 0) AS SINIESTRO_LIQUIDADO_TOTAL,
  IFNULL(s.CANTIDAD_SINIESTROS, 0) AS CANTIDAD_SINIESTROS,
  s.PERIODO_CONSULTADO AS PERIODO_SINIESTROS
...
Archivo Original: Siniestros +  Primas
