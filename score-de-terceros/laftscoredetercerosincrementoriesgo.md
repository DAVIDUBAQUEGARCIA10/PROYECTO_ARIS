# LAFT_Score_de_terceros_Incremento_Riesgo
...
WITH
-- 1) Normalización y orden de niveles
...
-- 2) Comparación contra el registro inmediatamente anterior por tercero
comparacion AS (
  SELECT 
    actual.*,
...
-- 2.1) Universo de pólizas / títulos vigentes
polizas_vigentes AS (
  SELECT
    CODIGO_COMPANIA,
...
-- ✅ 2.2) Restringidos SOLO CODIGO_RESTRICCION = 9 (dedupe para no multiplicar filas)
restringidos_9 AS (
  SELECT
    TIPO_DOCUMENTO_TERCERO,
    KEY_ID
  FROM `sb-ecosistemaanalitico-lago.seguros_bolivar.t_terceros_restringidos`
  WHERE CODIGO_RESTRICCION != 9
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY TIPO_DOCUMENTO_TERCERO, KEY_ID
    ORDER BY FECHA_MODIFICACION DESC, FECHA_CREACION DESC
  ) = 1
),

-- 3) Selección final con joins adicionales
seleccion_final AS (
  SELECT DISTINCT
    c.*,
    t.NOMBRE,
  ...
  LEFT JOIN (
    SELECT TIPO_DOCUMENTO, KEY_ID, FECHA_MODIFICACION, EXCEPTUADO_SARLAFT AS EXCEPTUADO_SARLAFT
    FROM `sb-ecosistemaanalitico-lago.seguros_bolivar.t_clientes_juridicos`
    UNION ALL
    SELECT TIPO_DOCUMENTO, KEY_ID, FECHA_MODIFICACION, "" AS EXCEPTUADO_SARLAFT
  ...
  -- 👉 LEFT JOIN pólizas / títulos vigentes
  ...
  -- 👉 LEFT JOIN restringidos SOLO con CODIGO_RESTRICCION = 9 (SIN agregar columnas al output)
  LEFT JOIN restringidos_9 r
...
WHERE rn_unico = 1 and CODIGO_RESTRINGIDO != 9 AND EXCEPTUADO_SARLAFT != "S"
AND NOT EXISTS (
  SELECT 1
...
Archivo Original: LAFT_Score_de_terceros_Incremento_Riesgo
