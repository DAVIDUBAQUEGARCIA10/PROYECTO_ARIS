...
INSERT INTO `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.cap_0010_prod`
(
  CODIGO_COMPANIA,
  TIPO_DOCUMENTO,
  KEY_ID_TOMADOR,
...
WITH resultado AS (
  ...
  WHERE p.VALOR_TRANSACCION > 0
    AND DATE(p.FECHA_TRANSACCION) >= DATE '2026-03-01'
...
WHERE m.rn_cliente = 1
  AND (
   -- UPPER(COALESCE(m.nivel_riesgo_jurisdiccion, '')) = 'ALTO'
    --OR 
    UPPER(COALESCE(m.nivel_riesgo_nacionalidad, '')) = 'ALTO'
  )
  AND NOT EXISTS (
    SELECT 1
    FROM `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.cap_0010_prod` t
...
Archivo Original: cap_0010_prod
