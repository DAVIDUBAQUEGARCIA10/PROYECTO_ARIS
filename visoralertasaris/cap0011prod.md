...
-- Solo considera títulos VIGENTES (ESTADO_TITULO = 1) según t_historico_titulos_capitalizacion.
-- Evita duplicados con NOT EXISTS (clave natural: cliente+doc+compañía+mes alerta).
-- ===========================================================

INSERT INTO `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.cap_0011_prod`
(
  CODIGO_COMPANIA,
  TIPO_DOCUMENTO,
  KEY_ID_TOMADOR,
...
WITH param AS (
  SELECT
    2  AS min_titulos_aporte,   -- >= 3 títulos con aportes
...
-- 0.1) Títulos vigentes según histórico de títulos
titulos_vigentes AS (
  SELECT DISTINCT
    NUMERO_TITULO
  FROM `sb-ecosistemaanalitico-lago.capitalizadora.t_historico_titulos_capitalizacion`
  WHERE ESTADO_TITULO = 1      -- solo títulos vigentes
),

-- 1) Base de movimientos + cliente por título (solo títulos vigentes)
base AS (
  SELECT
    p.CODIGO_COMPANIA,
    p.NUMERO_TITULO,
  ...
  -- Solo títulos vigentes
  JOIN titulos_vigentes tv
    ON tv.NUMERO_TITULO = p.NUMERO_TITULO
...
WHERE r.titulos_aporte_distintos >= (SELECT min_titulos_aporte FROM param)
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.cap_0011_prod` t
...
Archivo Original: cap_0011_prod
