...
INSERT INTO `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.cap_0015_prod` (
  CODIGO_COMPANIA,
  KEY_ID,
  TIPO_DOCUMENTO,
...
FROM pares u
WHERE rn = 1
  AND FECHA_APERTURA_NUEVO >= DATE '2026-03-01'
  AND NOT EXISTS (
    SELECT 1
    FROM `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.cap_0015_prod` t
...
Archivo Original: cap_0015_prod
