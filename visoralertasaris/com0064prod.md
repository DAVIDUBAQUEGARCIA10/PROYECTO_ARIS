...
```
INSERT INTO sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.com_0064_prod 
(
...
WITH SiniestrosConFila AS (
    ...
    WHERE 
        DATE(FECHA_SINIESTRO) >= '2026-03-01'  -- Solo siniestros desde 2024
        AND DATE_DIFF(DATE(FECHA_SINIESTRO), DATE(FECHA_SUSCRIPCION), DAY) <= 90  -- Siniestros en los primeros 90 días
        AND CODIGO_COMPANIA = 3  -- Compañía Comerciales Bolívar
...
WHERE 
    ...
    AND NOT EXISTS (
        SELECT 1
        FROM sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.com_0064_prod t
        WHERE t.KEY_ID_TOMADOR = SiniestrosConFila.KEY_ID_TOMADOR
          AND t.NUMERO_POLIZA = SiniestrosConFila.NUMERO_POLIZA
              AND t.LIQUIDADO_BOLIVAR = SiniestrosConFila.LIQUIDADO_BOLIVAR
    );
...
Archivo Original: com_0064_prod
