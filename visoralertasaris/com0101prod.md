...
```
-- INSERTAR NUEVOS REGISTROS EN COM_0101_VISOR_REPO CON FECHA DE ALERTA
INSERT INTO sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.com_0101_prod (
    KEY_ID_TOMADOR,
    CODIGO_RAMO_EMISION,
    CODIGO_COMPANIA,
    NOMBRE_RAMO_EMISION,
    ...
    FECHA_ALERTA_MES,
    FECHA_ALERTA,
    NUMEROS_POLIZA_LISTA,       -- << nueva columna
    CODIGOS_RAMO_EMISION_LISTA  -- << nueva columna
)
...
PolizasFiltradas AS (
    SELECT 
        ...
        STRING_AGG(DISTINCT CAST(p.NUMERO_POLIZA AS STRING), ', ' ORDER BY CAST(p.NUMERO_POLIZA AS STRING)) AS NUMEROS_POLIZA_LISTA,
        STRING_AGG(DISTINCT CAST(p.CODIGO_RAMO_EMISION AS STRING), ', ' ORDER BY CAST(p.CODIGO_RAMO_EMISION AS STRING)) AS CODIGOS_RAMO_EMISION_LISTA
    FROM 
        PolizasPorRamo t
    JOIN 
...
WHERE NOT EXISTS (
    SELECT 1
    FROM sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.com_0101_prod t
    WHERE 
        t.KEY_ID_TOMADOR = ConRiesgoAlto.KEY_ID_TOMADOR
        AND t.NUMERO_POLIZA = ConRiesgoAlto.NUMERO_POLIZA
);
...
Archivo Original: com_0101_prod
