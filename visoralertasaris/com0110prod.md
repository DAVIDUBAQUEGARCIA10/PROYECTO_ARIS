...
INSERT INTO sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.COM_0110_prod (
    CODIGO_COMPANIA,
    NUMERO_POLIZA,
...
WITH resultado AS (
    ...
    LEFT JOIN `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.t_jurisdicciones_nivel_riesgo` j
      ON c.CODIGO_CODAZZI_DIRECCION_1 = j.C__DIGO_DANE_DEL_MUNICIPIO

    WHERE 
        DATE(p.FECHA_FIN_POLIZA) >= CURRENT_DATE()
        AND p.MARCA_ANULACION != 'S'
        AND  j.NIVEL_DE_RIESGO_ASIGNADO = "Alto"
        AND CODIGO_RAMO_EMISION IN (923,924) 
),
...
con_riesgos AS (
    ...
    LEFT JOIN `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.t_actividad_economica_nivel_riesgo` ract
        ON n.CODIGO_CIIU = ract.C__digo

    LEFT JOIN `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.t_productos_nivel_riesgo` rprod
...
WHERE rn = 1
  AND NOT EXISTS (
    SELECT 1
    FROM sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.COM_0110_prod t
...
Archivo Original: com_0110_prod
