...
INSERT INTO sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0101B_prod
(
...
WITH SiniestrosConFila AS (
    ...
    WHERE 
        DATE(FECHA_SUSCRIPCION) >= '2026-03-01'  -- Pólizas suscritas desde el 2025
        ...
        AND DATE(FECHA_SINIESTRO) >= '2026-03-01'  -- Siniestros desde 2025
        AND CODIGO_RAMO_CONTABLE NOT IN (45, 34,40)  -- Exclusión de ciertos ramos contables
...
WHERE 
    ...
    AND NOT EXISTS (
        ...
        SELECT 1
        FROM sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0101B_prod existing
        WHERE existing.NUMERO_POLIZA = SiniestrosConFila.NUMERO_POLIZA
        AND existing.KEY_ID_TOMADOR = SiniestrosConFila.KEY_ID_TOMADOR
        AND existing.LIQUIDADO_BOLIVAR = SiniestrosConFila.LIQUIDADO_BOLIVAR
                AND existing.FECHA_ORDEN_PAGO = SiniestrosConFila.FECHA_ORDEN_PAGO
    );
...
Archivo Original: seg_0101B_prod
