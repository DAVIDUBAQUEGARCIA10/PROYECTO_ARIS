...
```
# Inserta nuevas alertas en SEG_0101_VISOR_REPO cuando el valor asegurado actual supera en más del 60% al anterior
INSERT INTO sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0101_prod (
    CODIGO_COMPANIA,
    CODIGO_RAMO_EMISION,
    NOMBRE_RAMO_EMISION,
    CODIGO_PRODUCTO,
...
WITH ranked_policies AS (
    ...
    FROM 
        (
            ...
            WHERE 
             FECHA_FIN_ENDOSO > "2026-03-01"
                AND VALOR_ASEGURADO > 0
                AND TIPO_DOCUMENTO_TOMADOR NOT IN ('NT', 'NE')
                AND CODIGO_RAMO_EMISION NOT IN (18, 20, 22, 24, 26, 29, 30, 33, 34, 42, 69, 70, 118, 10, 12, 66, 310,23,12,10,66,33,18,20,24,22,29,42)
    ...
    WHERE t1.SUM_VALOR_ASEGURADO > 1.60 * t2.SUM_VALOR_ASEGURADO
...
# Evitar duplicados ya presentes en la tabla
  AND NOT EXISTS (
    SELECT 1
    FROM sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0101_prod existing
...
Archivo Original: seg_0101_prod
