...
```
-- INSERTAR NUEVOS REGISTROS EN SEG_0103Z_PROD
INSERT INTO `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0103Z_prod` (
    KEY_ID_TOMADOR,
    CODIGO_RAMO_EMISION,
    CODIGO_COMPANIA,
    NOMBRE_RAMO_EMISION,
...
WITH PolizasPorRamo AS (
    ...
    WHERE 
        ...
        --AND FECHA_ORIGEN  > '2025-11-01'
        AND CAST(FECHA_FIN_POLIZA AS DATE) >= CURRENT_DATE()
        AND MARCA_ANULACION != 'S'
        AND CODIGO_COMPANIA = 2          -- ✅ si SIEMPRE es la 2, mejor filtrar aquí (más eficiente)
        AND CAST(FECHA_FIN_ENDOSO AS DATE) >= CURRENT_DATE()
        AND CAST(FECHA_FIN_POLIZA AS DATE) >= CURRENT_DATE()
        
    GROUP BY 
        KEY_ID_TOMADOR, CODIGO_COMPANIA, CODIGO_RAMO_EMISION
    HAVING 
        cantidad_polizas >= 3
        AND rango_dias <= 90
),
...
PolizasFiltradas AS (
    ...
    WHERE 
        p.FECHA_PROCESO > '2026-03-01'
        --AND p.FECHA_ORIGEN  > '2025-11-01'
        AND CAST(FECHA_FIN_ENDOSO AS DATE) >= CURRENT_DATE()
        AND CAST(FECHA_FIN_POLIZA AS DATE) >= CURRENT_DATE()
    ...
    GROUP BY 
        t.KEY_ID_TOMADOR, t.CODIGO_COMPANIA, t.CODIGO_RAMO_EMISION
),
...
WHERE NOT EXISTS (
    SELECT 1
    FROM `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0103Z_prod` t
    WHERE 
...
Archivo Original: seg_0103Z_prod
