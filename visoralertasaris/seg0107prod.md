...
```
INSERT INTO `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0107_prod` (
  CODIGO_COMPANIA,
  NUMERO_POLIZA,
...
WITH resultado AS (
  SELECT
    ...
    c.CODIGO_LUGAR_NACIMIENTO AS Codigo_municipio,
    c.NOMBRE_LUGAR_REGISTRO AS municipio_direccion_1,
  ...
  WHERE
    ...
    AND (
      nat.Nivel_de_riesgo_asignado  in ('Alto')
      OR j.NIVEL_DE_RIESGO_ASIGNADO  in ('Alto')
    )
    AND p.CODIGO_COMPANIA = 2
    and CODIGO_PRODUCTO  not in (66,67)
),
...
WHERE
  ...
  --AND nivel_riesgo_producto = 'Alto'
    AND (
      nivel_riesgo_jurisdiccion in ('Alto')
      OR nivel_riesgo_nacionalidad in ('Alto')
    )
  AND NOT EXISTS (
    SELECT 1
    FROM `sb-ecosistemaanalitico-lago.cumplimiento_normativo_prod.seg_0107_prod` t
...
Archivo Original: seg_0107_prod
